...
The LFortran compiler has made great strides over the last few years. In 2020, it was [missing a lot of features](https://gitlab.com/lfortran/lfortran/-/issues/121) and only supported a very small subset of Fortran. Now it now supports a much wider range of language features and can be used to compile a reasonable amount of Fortran code. It can even compile to WebAssembly out of the box!^[Check out the LFortran demo at <https://dev.lfortran.org>. While extremely impressive, note that the first thing I tried was changing `x ** 2` to `x ** 3` and saw that such a change is currently not supported by the code generator.]
...
Grab a cuppa^[**cuppa** (/ˈkʌp.ə/): noun, informal, UK. A hot drink, usually tea or coffee.], this step uses a lot of resources and can take a __very__ long time.
...
And there we have it: BLAS compiled from Fortran 90 sources and running under WebAssembly! To finish up, let’s confirm for ourselves that this output is correct^[Keeping in mind Fortran's column-major array layout.],
